...
assert dataset_dir is not None, (
    "Specify 'TAU Urban Acoustic Scenes 2022 Mobile dataset' location in variable "
...
dataset_config = {
    ...
    "eval_dir": os.path.join(
        dataset_dir, "..", "TAU-urban-acoustic-scenes-2024-mobile-evaluation"
    ),
    "eval_meta_csv": os.path.join(
        dataset_dir,
        "..",
        "TAU-urban-acoustic-scenes-2024-mobile-evaluation",
        "meta.csv",
    ),
}
...
class BasicDCASE24Dataset(TorchDataset):
    ...
    def __init__(self, meta_csv):
        ...
        self.cities = le.fit_transform(
            df["identifier"].apply(lambda loc: loc.split("-")[0]).values.reshape(-1)
        )
...
def get_training_set(split=100, roll=False):
    assert str(split) in (
        ...
        "50",
        "100",
    ), "Parameters 'split' must be in [5, 10, 25, 50, 100]"
...
def get_base_training_set(meta_csv, train_files_csv):
    ...
    ds = SimpleSelectionDataset(BasicDCASE24Dataset(meta_csv), train_subset_indices)
    return ds
...
def get_test_set():
    ...
    test_split_csv = os.path.join(
        dataset_config["split_path"], dataset_config["test_split_csv"]
    )
...
def get_eval_set():
    assert os.path.exists(
        dataset_config["eval_dir"]
    ), f"No such folder: {dataset_config['eval_dir']}"
